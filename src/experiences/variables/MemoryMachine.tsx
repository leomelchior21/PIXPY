...
export function MemoryMachine({ progress, onProgress, onBack }: Props) {
  ...
  return <ExperienceShell order="04" title="Memory Machine" question="Where does a variable's value go?" accent="#a994ff" hints={['Watch the glowing code line.', 'The middle window shows what Python remembers now.', 'print() sends a remembered value to output.']} completed={completed} objective="Send values from code to memory, then reveal them in output." onBack={onBack} onReset={resetLevel} className="memory-experience memory-story-experience">
