...
async function checkClassroomInteractions(send, size) {
  ...
  await type('#story-age', '12')
  await click('.age-entry button')
  await waitFor(send, `Boolean(document.querySelector('.age-demo-machine.is-done'))`)
  await click('.story-next')
  await click('.input-quick-quiz button:nth-child(2)')
  await click('.story-next')
  await click('.story-next')
  ...
  await waitFor(send, `document.querySelector('.chat-message.result-success p')?.innerText === '18'`, 15000)
  await check('Story flow and Enter send real input', `document.querySelector('.chat-message.result-success p').innerText === '18'`)
  await type('#machine-input', 'hello')
  await click('.phone-composer button')
  ...
  await check('Reset clears the selected machine', `document.querySelector('#machine-input').value === '' && !document.querySelector('.chat-message.result-error')`)
  await type('#machine-input', '8')
  await click('.phone-composer button')
  await waitFor(send, `document.querySelector('.chat-message.result-success p')?.innerText === '16'`, 15000)
