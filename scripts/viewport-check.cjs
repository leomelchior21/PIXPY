...
async function checkMemoryQuiz(send, size) {
  ...
  fs.writeFileSync(path.join(outputDir, `memory-mode-intro-${size.name}.png`), Buffer.from(introScreenshot.data, 'base64'))
  ...
  await waitFor(send, `document.querySelector('.memory-run')?.innerText.includes('EXECUTE LINE 2')`, 30000)
  const lineScreenshot = await send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  fs.writeFileSync(path.join(outputDir, `memory-line-one-${size.name}.png`), Buffer.from(lineScreenshot.data, 'base64'))
  await send('Runtime.evaluate', { expression: `document.querySelector('.memory-run')?.click()` })
  ...
  await send('Runtime.evaluate', { expression: `document.querySelector('.memory-chapter-quiz .chapter-quiz-next')?.click()` })
  await sleep(100)
  const finishScreenshot = await send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  fs.writeFileSync(path.join(outputDir, `memory-quiz-finish-${size.name}.png`), Buffer.from(finishScreenshot.data, 'base64'))
...
async function checkClassroomInteractions(send, size) {
  ...
  await click('.mode-intro .story-next')
  await type('#machine-input', 'hello')
  await click('.phone-composer button')
  await waitFor(send, `document.querySelector('.chat-message.result-success p')?.innerText === 'hello'`, 15000)
  await type('.cm-content', '# Try replacing input() with your name in quotes\nmessage = "Maya"\nprint(message)')
  await click('.input-run')
  await waitFor(send, `Boolean(document.querySelector('.input-coachmark.is-catch'))`, 15000)
  await type('.cm-content', '# Try replacing input() with your name in quotes\nmessage = input()\nprint(message)')
  await click('.input-run')
  await waitFor(send, `!document.querySelector('.input-next').disabled`, 15000)
  await type('#machine-input', '9')
  await press('Enter', 'Enter', 13)
  await waitFor(send, `document.querySelector('.chat-message.result-success p')?.innerText === '9'`, 15000)
  await check('Enter sends real input', `document.querySelector('.chat-message.result-success p').innerText === '9'`)
  await type('.cm-content', 'message = int(input())\nprint(message)')
  await type('#machine-input', 'hello')
  await click('.phone-composer button')
  await waitFor(send, `Boolean(document.querySelector('.chat-message.result-error'))`, 15000)
  await check('Invalid input produces readable error', visible('.chat-message.result-error'))
  await click('.input-code-panel .secondary-action')
  await check('Reset clears the input lab', `document.querySelector('#machine-input').value === '' && !document.querySelector('.chat-message.result-error')`)
  await type('#machine-input', '8')
  await click('.phone-composer button')
  await waitFor(send, `document.querySelector('.chat-message.result-success p')?.innerText === '8'`, 15000)
  ...
  await click('.input-next')
  await click('.chapter-quiz-options button:nth-child(2)')
  await capture('input-chapter-quiz')
  await click('.chapter-quiz-next')
  await click('.chapter-quiz-options button:nth-child(2)')
  await click('.chapter-quiz-next')
  await click('.chapter-quiz-options button:nth-child(1)')
  await click('.chapter-quiz-next')
  await waitFor(send, `Boolean(document.querySelector('.input-quiz-finish'))`)
  await check('Quiz finish hands off to Memory Machine', visible('.input-quiz-finish .next-experiment-button'))
  await click('.input-quiz-finish .next-experiment-button')
  await waitFor(send, `Boolean(document.querySelector('.memory-story'))`)
  ...
  await click('.memory-run')
  await waitFor(send, `Boolean(document.querySelector('.memory-try-popup'))`, 30000)
  await check('The memory monitor shows the stored variable', `document.querySelector('.memory-monitor-node').classList.contains('has-value') && document.querySelector('.memory-monitor-screen').innerText.includes('x')`)
